...
function setup() {
  ...
  for (let bacNum = 0; bacNum < 20; bacNum++) {
    let thisX = random(200, 800);
    let thisY = random(150, 400);
    bacArray[bacNum] = new bacteria(thisX, thisY, 50);
  }
...
function draw() {
  ...
  for (let bananas = 0; bananas < squares.length; bananas++) {
    squares[bananas].move();
    squares[bananas].create();
  }
  for (let j = 0; j < bacArray.length; j++) {
    if (isTouching(squares[bananas], bacArray[j])) {
      bacArray.splice(j, 1); // remove the bacteria
      bacDrawn--; // decrement the number of drawn bacteria
      break; // break out of the inner loop after removing the bacteria
    }
  }
  ...
  for (let bacNum = 0; bacNum < bacDrawn; bacNum++) {
    bacArray[bacNum].create();
    bacArray[bacNum].move();
    // the j in this loop is called "matesChecked" the tutorial video
    if (framesDelayed > spawnDelay) {
      for (let j = bacNum + 1; j < bacArray.length; j++) {
        let isDifferentBac = bacDrawn !== j;
        let distance = dist(
          bacArray[bacNum].x,
          bacArray[bacNum].y,
          bacArray[j].x,
        ...
        if (isDifferentBac && distance <= spriteWidth) {
          let newX = (bacArray[bacNum].x + bacArray[j].x) / 2;
          let newY = (bacArray[bacNum].y + bacArray[j].y) / 2;
           let thisX = random(100, 860);
          let thisY = random(128, 410);
          framesDelayed = 0;
          bacArray.push(new bacteria(newX,newY));
        }
      }
    }
  }
